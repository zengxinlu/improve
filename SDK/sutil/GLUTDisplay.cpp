...
bool           GLUTDisplay::m_display_fps          = false;
...
unsigned int   GLUTDisplay::m_warmup_frames        = 5u;//50u;
...
void GLUTDisplay::keyPressed(unsigned char key, int x, int y)
{
	...
	case 'f':
		if ( m_old_window_width == -1) { // We are in non-fullscreen mode
			...
			m_old_window_x      = glutGet(GLUT_WINDOW_X);
			m_old_window_y      = glutGet(GLUT_WINDOW_Y);
			glutFullScreen();
